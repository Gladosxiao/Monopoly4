...
describe('calculateRent', () => {
  ...
  it('住宅：基础租金 + 等级系数', () => {
    ...
    expect(calculateRent(state.map.tiles[1], owner, state, visitor).rent).toBe(45); // 30 * 1.5
  });
  ...
  it('住宅：同组 2 块加成 20%', () => {
    ...
    const owner = state.players[0];
    const visitor = state.players[1];
    expect(calculateRent(state.map.tiles[1], owner, state, visitor).rent).toBe(36); // 30 * 1.2
  });
  ...
  it('连锁店：按全图连锁店数量联合收费', () => {
    ...
    const owner = state.players[0];
    const visitor = state.players[1];
    expect(calculateRent(state.map.tiles[1], owner, state, visitor).rent).toBe(90); // 30 * 3
  });
  ...
  it('商场：baseRent * level * 转盘倍数', () => {
    ...
    const owner = state.players[0];
    const visitor = state.players[1];
    expect(calculateRent(state.map.tiles[21], owner, state, visitor).rent).toBe(240); // 120 * 2 * 1
  });
  ...
  it('大穷神：过路费翻倍', () => {
    ...
    expect(calculateRent(state.map.tiles[1], owner, state, visitor).rent).toBe(60); // 30 * 2
  });
  ...
  it('小财神：过路费减半', () => {
    ...
    expect(calculateRent(state.map.tiles[1], owner, state, visitor).rent).toBe(15); // 30 * 0.5
  });
  ...
  it('涨价卡：指定路段翻倍', () => {
    ...
    const owner = state.players[0];
    const visitor = state.players[1];
    expect(calculateRent(state.map.tiles[1], owner, state, visitor).rent).toBe(72); // 30 * 1.2 * 2
  });
