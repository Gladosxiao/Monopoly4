...
### 决策优先级（从高到低，不可违反）
1. **如果 availableActions 中有 buyProperty：必须选择 buyProperty**，除非你买完地后现金会低于总资产的 10%。这是最高优先级，高于一切卡片/道具/股票操作。
2. **如果 availableActions 中有 upgradeProperty 且你现金≥升级费用+总资产20%：选择 upgradeProperty**，升级带来长期租金收益。
...
- **目标是 50 回合内让地图上 80% 以上的空地被购买**。如果 50 回合后购买率<80%，说明策略失败。
- 优先购买同组空地产以形成垄断（同组 2 块租金+20%，3 块+50%），并尽快升级（每级+50%租金）。
- 买地时保留至少总资产 10% 的现金即可，不要把现金囤积在手里不买地。

### 资产配置纪律
- 理想比例：地产 50%-70%，现金 20%-30%，股票 10%-20%。
- **地产少于 4 块时，禁止买入股票**。只有地产≥4 块且买完股票后现金仍高于总资产 30% 时，才可买入股票。
- 不要把全部现金用于股票或升级，确保有足够的地产基础。
...
- 攻击卡：对对手高级地产使用涨价卡/查封卡/恶魔卡/怪兽卡/拆除卡；对资金最多者使用均贫卡/陷害卡/冬眠卡/梦游卡/乌龟卡/转向卡/停留卡。
...
2. **如果列表中有 buyProperty，必须优先选择 buyProperty**（买完地后现金不得低于总资产10%）。
3. **如果列表中有 roll，必须选择 roll**（或使用遥控骰子），不能用 tradeStock/useCard/useItem 代替掷骰子。
...
const PERSONALITY_HINTS: Record<PlayerPersonality, string> = {
  'risk-loving': '你是风险偏好型玩家：敢于借贷、敢于梭哈股票、敢于攻击对手；现金保留线可低至总资产10%。',
  'risk-averse': '你是风险规避型玩家：优先保现金、不借贷、不激进炒股、以稳健买地收租为主；现金保留线至少总资产30%。',
  'property-focus': '你是地产专注型玩家：把主要精力放在买地、升级、形成垄断上；股票只作为现金冗余时的配置，卡片优先用于保护/扩张地产。',
  'stock-focus': '你是股市专注型玩家：把主要精力放在低买高卖股票、争夺董事长上；地产只买关键地块，现金优先投入股市。',
};
...
## 行动优先级提醒
- 如果上方"本回合可用操作"中有 buyProperty：必须选择 buyProperty（买完地后现金不低于总资产10%即可）。
- 如果可用操作中有 roll：必须选择 roll 或使用遥控骰子，不能用股票/卡片/道具代替掷骰子。
- 在买地/掷骰之后，再考虑攻击卡/陷阱/股票/商店。
