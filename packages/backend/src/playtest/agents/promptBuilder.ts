...
2. **地产专注型**：看到 buyProperty 就买（保留10%现金即可）；落在自己地产且现金够时，优先 upgradeProperty。
...
- 优先购买同组空地产以形成垄断（同组 2 块租金+20%，3 块+50%），并尽快升级（每级+50%租金）。同组垄断后务必把该组地产升到满级，租金才会显著提高。
- **组合策略**：如果同组空地已被对手买走，优先使用换地卡/换房卡夺取；或在商店购买能换地/换房的卡片。
- 升级优先级：同组垄断的地产 > 差1块形成垄断的地产 > 自己经常路过的地产 > 其他地产。
...
### 遥控骰子（主动策略）
- **优先使用遥控骰子落点自己的地产进行升级**，特别是已经形成同组垄断或差 1 块就垄断的地产。
- 其次：前方 1-6 格内有空地产且资金够。
- **再次：若你当前卡片少于 5 张、道具总数少于 3 个、或点券 ≥80，优先用遥控骰子落点前方商店补充资源**；前方有小游戏也可前往获取点券。
- 没有明确目标时直接 roll 普通骰子。

### 点券与商店
- **商店是中期资源补给点**：站在商店且点券充足时，优先补充攻击卡（涨价、查封、怪兽、拆除、陷害、冬眠）和陷阱（地雷、路障），保持手牌/道具池在健康水平（至少 5 张卡、3 个道具）。
- 不要因逛商店而错过买地；但如果当前格就是商店，应在离开前至少购买 1 张有用卡片或 1 个陷阱。
`;
