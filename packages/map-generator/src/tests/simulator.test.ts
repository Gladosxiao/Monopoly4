...
describe('simulateMap', () => {
  it('模拟 PLAYER4 模板人均卡片+道具约 10 个', () => {
    ...
    assert.ok(result.avgTotalCardsAndItemsPerPlayer >= 9, `人均卡+道 ${result.avgTotalCardsAndItemsPerPlayer} 小于 9`);
    assert.ok(result.avgTotalCardsAndItemsPerPlayer <= 13, `人均卡+道 ${result.avgTotalCardsAndItemsPerPlayer} 大于 13`);
  });

  it('模拟 MAP80 模板人均点券约 580', () => {
    ...
    assert.ok(result.avgCouponsPerPlayer >= 500, `人均点券 ${result.avgCouponsPerPlayer} 小于 500`);
    assert.ok(result.avgCouponsPerPlayer <= 700, `人均点券 ${result.avgCouponsPerPlayer} 大于 700`);
  });
  ...
  it('机车（1-2 颗骰子）移动更快且仍能获得卡片道具', () => {
    ...
    assert.ok(result.avgTotalCardsAndItemsPerPlayer >= 9, `机车人均卡+道 ${result.avgTotalCardsAndItemsPerPlayer} 不足`);
  });
  ...
  it('汽车（1-3 颗骰子）移动最快且仍能获得卡片道具', () => {
    ...
    assert.ok(result.avgTotalCardsAndItemsPerPlayer >= 9, `汽车人均卡+道 ${result.avgTotalCardsAndItemsPerPlayer} 不足`);
  });
