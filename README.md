...
> **本项目是一个纯 Vibe Coding 项目**，由 AI（Claude）全程辅助生成代码、设计文档、测试用例。
>
> Vibe Coding 的特点是"跟着感觉走"，追求快速出活、快速迭代。因此代码中可能存在：
> - **不一致**：文档与实现脱节、命名风格不统一、重复代码未及时清理。
> - **过度设计**：某些模块可能为了"看起来完整"而引入不必要的复杂度。
> - **遗漏**：边界条件、错误处理、类型安全等可能不够严谨。
> - **"能跑就行"**：优先保证功能可用，代码质量、性能优化、可维护性暂时搁置。
>
> 这是 Vibe Coding 的固有特性，不是 bug。如果你发现问题，欢迎指出，我会（在 AI 的帮助下）修复。
