...
代码实现（状态机）：
- `GameState.status: 'rolling' | 'moving' | 'acting' | 'minigame' | 'ended'` —— 当前阶段（走到小游戏格时进入 `minigame`）。
...
《大富翁4》原版共 **30 张卡片**，当前全部 30 张卡片效果均已落地。
...
| 20 | 嫁禍卡 | 防御 | 将本应自己承担的罚金/损失嫁祸给指定对手。价格 40 点券。
...
| 25 | 紅卡 | 金融 | 令指定公司股票连涨 3 天（涨停）。价格 30 点券。
...
- 价格资料与截图：`doc/card_item_pricing.md`、`doc/pricing_screenshots/`

> 以下价格来自游戏实际商店截图（见 `doc/pricing_screenshots/`），未在截图中出现的卡片保留常见默认值。
...
| 恶魔卡 | 180 | 飞弹 | 100 |
| 怪兽卡 | 60 | 核子飞弹 | 0（研究所产物） |
| 拆除卡 | 15 | 机器人/时光机/传送机/工程车 | 0（研究所产物） |
...
> 价格调整只需修改 `cards.ts` / `items.ts` 中的 `cost` 字段，设计文档和价格资料需同步更新。
...
- 玩家可随时打开股票交易界面买卖上市股票。
- 股票列表字段包括：**股票名称、成交价、涨跌、持有股数、成本价（仓位）、浮动盈亏**。
- 交易规则：
  - 买入/卖出以当前成交价成交，现金自动从银行存款扣除/存入。
  - 每家公司总股本 10000 股；买入按加权平均更新成本价，卖出减仓保留成本，清仓删除仓位记录。
  - 股价随机波动，设涨停/跌停限制；新闻事件可影响特定行业或全部股价。
- **大股东与董事长**：
  - 需持有该公司总股本的 **>10%** 才能成为董事长；持股最多者当选，张数相同则保留原董事长。
