...
```
balloon:    平均点券=103.5, 标准差=18.0, 范围=[38, 166], 平均操作=74.0, 平均命中=27.3
luckyDrop:  平均点券=114.4, 标准差=110.8, 范围=[0, 500], 平均操作=14.1, 平均命中=12.7
penguinDig: 平均点券=114.1, 标准差=51.8, 范围=[0, 267], 平均操作=54.0, 平均命中=54.0
```

三个游戏的随机玩家期望点券均在 **100+**（三游戏平均 110.7），达到设计目标。若使用 `run-calibrator.ts` 基于默认模拟结果做一次示例标定，再用标定参数仿真验证，三游戏平均点券可收敛到约 **112**。
