...
> 当前版本已实现完整的可玩闭环：掷骰、移动、买地、升级、改建、过路费扩展、30 张卡片/13 种道具、事件/新闻/股票/公司/保险系统、移动阶段、陷阱触发、NPC、医院格特效、土地权限到期、贷款/还款、多地图加载、乐透/魔法屋、神明变身规则、胜利条件完整检查以及保险骗保联动。剩余可细化项主要包括更复杂的神明效果、小游戏扩展、12 名角色的属性差异与更完整的破产法拍策略。
...
| 7 | 破产与获胜 | 90% | 破产判定/法拍/资金目标/时间限制/唯一幸存者均已实现 |
| 8 | 行走与回合 | 85% | `moving` 阶段、逐格移动、`onPassTile`（起点工资/陷阱/NPC）已实现 |
| 9 | 卡片系统 | 95% | 30 张效果已全部落地（部分简化），卡片格随机发卡已接入 |
| 10 | 道具系统 | 95% | 13 种效果已全部落地（部分简化），陷阱触发已接入移动过程 |
| 11 | 神明附身系统 | 60% | 定义、租金效果、地图神明与请神符寻路已实现，神明到期消失已实现，变身规则占位 |
| 12 | 股票、公司与保险 | 85% | 股票交易/董事长/公司地块特效/保险购买与理赔已实现；住院自动理赔联动/商场旅馆转盘结果已记录 |
| 13 | 其他系统（特殊地点/小游戏/NPC） | 40% | NPC（四大恶人/恶犬/乞丐）、陷阱触发、医院格特效已实现；乐透/魔法屋仍占位 |
...
1. **近期优先**：
   ...
   - 实现土地权限到期（`landLease`、`purchasedAt`、`expiresAt`）与贷款/还款。
2. **中期目标**：胜利条件完整检查、乐透/魔法屋、神明变身规则、多地图加载。
3. **长期扩展**：多地图加载、更复杂的 NPC AI、小游戏扩展。
