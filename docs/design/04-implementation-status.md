...
> 当前版本已实现完整的可玩闭环：掷骰、移动、买地、升级、改建、过路费扩展、30 张卡片/13 种道具、事件/新闻/股票/公司/保险系统、移动阶段、陷阱触发、NPC、医院格特效、土地权限到期、贷款/还款、多地图加载、乐透/魔法屋、神明变身规则、胜利条件完整检查以及保险骗保联动。后端测试 331+ 用例，金融/事件/商店等模块覆盖率显著提升。剩余可细化项主要包括更复杂的神明效果（福神送卡/买地半价等）、小游戏扩展、12 名角色的属性差异与更完整的破产法拍策略。
...
| 2 | 角色属性 | 80% | 现金/储蓄/贷款/点券/股票/保险/总资产计算/破产法拍均已实现；12 角色属性差异待补充 |
| 3 | 地图与路径 | 95% | `@monopoly4/map-generator` 已提供多模板、加载器、坐标工具、SVG/HTML 渲染与棋子占位；前端 `board.ts` 已接入坐标工具 |
| 4 | 土地类型 | 75% | 大小块/建筑类型/特殊建筑租金均已落地，研究院产物未实现 |
| 5 | 土地购买与升级 | 80% | 购买/升级/改建可用；大块土地默认建住宅，建造费用简化 |
| 6 | 过路费 | 90% | 住宅/连锁店/特殊建筑/神明/卡片影响均已实现 |
| 7 | 破产与获胜 | 95% | 破产判定/法拍/资金目标/时间限制/唯一幸存者/月度结算均已实现 |
| 8 | 行走与回合 | 90% | `moving` 阶段、逐格移动、`onPassTile`/`onArriveTile`/状态递减均已实现 |
| 9 | 卡片系统 | 95% | 30 张效果已全部落地（部分简化），商店/卡片格/出售均已接入 |
| 10 | 道具系统 | 95% | 13 种效果已全部落地（部分简化），陷阱触发/商店/出售均已接入 |
| 11 | 神明附身系统 | 75% | 定义、租金效果、地图神明、请神符寻路、到期变身/消失均已实现；福神送卡/买地半价等效果待细化 |
| 12 | 股票、公司与保险 | 90% | 股票交易/董事长/公司地块特效/保险购买与理赔/住院自动理赔联动/商场旅馆转盘均已实现 |
| 13 | 其他系统（特殊地点/小游戏/NPC） | 75% | NPC、陷阱触发、医院格特效、乐透/魔法屋、点券格均已实现；小游戏待扩展 |
| 14 | 命运与新闻事件 | 80% | 事件注册表与效果描述符已实现，覆盖主要命运/新闻事件；地图限定/神明挡灾待细化 |
...
| 破产法拍（3 次） | 需要 | 支付过路费/金钱不足导致破产时，地产转移给债主；完整 3 次法拍策略待细化 | ⚠️ 部分实现 |
...
| 研发产物 | 机器人/时光机/传送机/工程车/核子飞弹 | 5 种均已落地（时光机为上回合快照、工程车拆除经过土地） | ✅ 已实现 |
...
**代码位置**：`packages/shared/src/data/spirits.ts`、`packages/backend/src/game/engine.ts`（decrementEffects、getSpiritRentMultiplier、useCard summonSpirit/dismissSpirit）
...
| 小游戏 | 已实现 | ✅ 已实现 |
| 特殊 NPC（四大恶人/恶犬/乞丐） | `npcSystem/` 生成/移动/效果已接入移动过程 | ✅ 已实现 |

**代码位置**：`packages/backend/src/game/engine.ts`（movePlayer、handleTileEffect）、`packages/backend/src/game/itemSystem/trapSystem.ts`、`packages/backend/src/game/npcSystem/index.ts`
...
1. **近期优先**：
   - 细化神明附身完整效果（福神送卡/买地半价、土地公守护建筑等）。
   - 补充 12 名角色的属性差异（初始现金、投资偏好、移动能力等）。
   - 完善破产法拍策略：变卖股票/土地顺序、3 次法拍机会、玩家间交易/拍卖。
2. **中期目标**：
   - 小游戏扩展（七彩气球、喜从天降、企鹅挖宝）。
   - 地图限定事件与神明挡灾/加倍倍率细化。
   - 更复杂的路段效果、研究院产物研发逻辑。
3. **长期扩展**：
   - AI 托管与单机对战机器人。
