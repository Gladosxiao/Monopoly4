...
> 当前版本已实现最基础的可玩闭环（掷骰、移动、买地、升级、住宅过路费、破产判定），并补全了过路费扩展、卡片/道具系统、事件/新闻/股票/公司/保险系统，以及移动阶段、陷阱触发、NPC（四大恶人/恶犬/乞丐）与医院格特效。剩余主要缺口为研究所产物、乐透/魔法屋、部分卡片占位与神明变身规则。
...
| 10 | 道具系统 | 80% | 13 种已定义并接入 `itemSystem`；交通工具/陷阱/工具类效果已落地（陷阱触发已接入移动过程），5 种研究所产物占位 |
...
1. **近期优先**：
   ...
   - 补齐卡片与道具剩余占位效果（均富/查税/红黑卡/研发产物等）。
2. **中期目标**：实现土地权限到期（`landLease`、`purchasedAt`、`expiresAt`）、胜利条件完整检查、乐透/魔法屋、神明变身规则。
