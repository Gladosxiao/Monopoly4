...
> 当前版本已实现完整的可玩闭环：掷骰、移动、买地、升级、改建、过路费扩展、30 张卡片/13 种道具、事件/新闻/股票/公司/保险系统、移动阶段、陷阱触发、NPC、医院格特效、土地权限到期、贷款/还款、多地图加载、乐透/魔法屋、神明完整效果（财神/穷神租金、福神买地打折/送卡、衰神买地/商店破坏、天使/恶魔住院入狱天数、土地公守护建筑）、土地公/天使 100% 挡灾、破产前 3 次法拍、3 个小游戏接入、胜利条件完整检查、保险骗保联动。近期新增 UI/UX 优化：事件 Banner 弹窗、车辆道具装备/卸下双状态、机器人道具升级/改建分支询问、12 个 emoji 棋子 PNG、地块独立色彩体系/深色顶部标题栏/所有者标识/等级数字显示、股票持股比例。后端测试 389 用例，金融/事件/商店/神明/小游戏/破产法拍/道具等模块覆盖率高。剩余可细化项主要包括 12 名角色的属性差异、地图限定事件、更复杂的前端移动动画与建筑升级特效。
...
| 5 | 土地购买与升级 | 92% | 购买/升级/改建可用，福神/衰神已影响买地/升级费用；机器人道具支持先选土地再选择升级/改建分支；地块所有者标识与等级数字已在前端渲染 |
...
| 前端地块标题栏 | property pastel 纯色、functional 浅灰白底 + 高饱和度类型色加粗描边；顶部约 28% 深色标题栏显示白色名称（property 为分组色加深、functional 为类型色），底部带区分色细线；所有者标识位于标题栏下方；棋子/神明/陷阱避开标题栏 | ✅ 已实现 |
