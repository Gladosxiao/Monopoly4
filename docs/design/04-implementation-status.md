...
> 当前版本已实现最基础的可玩闭环（掷骰、移动、买地、升级、住宅过路费、破产判定），并在本次迭代中补全了过路费扩展（连锁店、特殊建筑、神明、卡片）。大量扩展系统尚未实现。
...
| 商店购买 | 点券购买，15 张上限 | `buyCard` 已实现基础版（无商店格限制） | ⚠️ 部分实现 |
| 卡片格获得 | 经过获得随机卡片 | 已实现 | ✅ 已实现 |
| 使用卡片 | 30 张效果 | 已落地核心子集，其余待扩展 | ⚠️ 部分实现 |
| 卡片效果注册表 | `CardEffectRegistry` | 无独立注册表，效果直接写在 `useCard` switch | ❌ 未实现 |

**代码位置**：`packages/shared/src/data/cards.ts`、`packages/backend/src/game/engine.ts`（useCard、buyCard、sellCard）
...
| 交通工具/陷阱/工具/研发产物 | 需要 | 无实际逻辑 | ❌ 未实现 |
| 道具使用入口 | `useItem` | 仅占位返回未实现 | ❌ 未实现 |

**代码位置**：`packages/shared/src/data/items.ts`、`packages/backend/src/game/engine.ts`（useItem/buyItem/sellItem 占位）
...
| 命运/机会格 | 简化为随机 -2000~+3000 现金事件 | ⚠️ 临时实现 |
| 税务格 | 固定 -5000 | ⚠️ 临时实现 |
| 卡片格 | 经过随机获得一张卡片 | ✅ 已实现 |
