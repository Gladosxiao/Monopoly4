...
- 每家公司对应一只股票，总股本 10000 股，流通股即为总股本。
- 玩家可随时买入/卖出，现金优先、存款兜底。
- 每次买入按加权平均记录**成本价（仓位）**：
  - `新成本 = (旧成本 × 旧股数 + 成交价 × 买入股数) / 总持股数`
  - 卖出减仓时保留成本价；全部清仓后删除该股票的仓位记录。
- 每日收盘后股价随机波动 ±10%。
- 停牌期间不可交易。
- 每月 15 日按公司累计盈余的 10% 分红。
- **董事长**：需持有该公司总股本的 **>10%**；持股最多者当选，同股数时保留原董事长。
...
- `packages/frontend/src/main.ts`：
  - 右侧新增「股市与公司」面板，展示股价、涨跌、持有量、成本价（仓位）、浮动盈亏、董事长。
  - 玩家信息增加保险天数。
  - 操作区增加改建、使用卡片、申请理赔按钮。
...
- `packages/backend/src/game/__tests__/financial.test.ts`：39 个用例覆盖股票交易、加权成本、董事长、分红、股价波动、停牌、公司特效、保险理赔。
