...
### 9.1 预研：5 局 50 回合快速验证

> 运行命令：`MAX_TURNS=50 PLAYTEST_SLEEP_MS=0 npx tsx src/playtest/run5heuristic.ts`
...
|---|---|---|---|---|---|---|
| 结果 | max-turns | max-turns | max-turns | max-turns | max-turns | - |
| 回合数 | 50 | 50 | 50 | 50 | 50 | 50 |
| 破产数 | 1 | 2 | 0 | 0 | 0 | 0.6 |
| 土地购买率 | 66% | 55% | 58% | 63% | 55% | 59.4% |
| 商店访问率 | 4.1% | 14.3% | 4.1% | 6.1% | 4.1% | 6.5% |
| 股票总盈亏 | +$200 | -$1,002 | -$4,692 | $0 | -$2,250 | -$1,549 |
| 攻击行为数 | 9 | 12 | 13 | 11 | 12 | 11.4 |

**50 回合观察**：
- 买地机制使用充分，平均近 60% 空地被购入，符合预期。
- 攻击卡片/道具已被积极使用（每局 9-13 次），且 100% 指向最富有对手。
- 股票有赢有亏，但多数局亏损，说明当前股价波动下 AI 择时策略仍有优化空间。
- 商店访问率偏低（4-14%），expanded 地图有 6 个商店格，但 AI 经常因现金保留策略或落点随机性而错过。
- 50 回合平均破产数仅 0.6，意味着部分对局到 200 回合仍可能无人破产，需要 200 回合长对局进一步验证。

### 9.2 200 回合长对局验证

> 运行命令：`MAX_TURNS=200 PLAYTEST_SLEEP_MS=0 npx tsx src/playtest/run5heuristic.ts`
> 配置同上。

（待 5 局 200 回合运行完成后补充完整表格与调参结论）

### 9.3 已发现的问题与修复

1. **Playtest 批量对局运行缓慢**：原 `freePlay.ts` 主循环每次动作后固定 `sleep(100)`，且状态等待超时 3-5s、检测间隔 30ms，导致 200 回合对局极慢。
   - **修复**：`PLAYTEST_SLEEP_MS` 环境变量覆盖 sleep 时间；`waitForLatestStateChange` 默认超时改为 1000ms，检测间隔改为 5ms。本地 50 回合对局耗时从数分钟降至约 150-200s。
2. **地图生成器 dist 未同步**：`@monopoly4/map-generator` 的 `dist/` 产物为旧版本，导致实际加载的地图缺少 `miniGame` 格。
   - **修复**：执行 `npm run build -w packages/map-generator`，确认所有注册地图均包含 1 个小游戏格，且 `baseRent/basePrice ≈ 0.1`。
3. **50 回合部分对局无破产**：200 回合是否每局都出现破产仍需验证。若出现 200 回合无人破产，将考虑：
   - 提高 `rentMultiplier` 1.5 → 2.0，加快租金压力；
   - 或降低 `salary` 3000 → 2000，减少工资安全网；
   - 或提高 AI 买地/升级激进程度，让垄断更快形成。
