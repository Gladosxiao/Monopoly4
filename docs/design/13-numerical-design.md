...
> 运行命令：`MAX_TURNS=200 PLAYTEST_SLEEP_MS=0 ROUND_INDEX=200-N npx tsx src/playtest/run1heuristic.ts`（5 局并行）。
> 配置：`rentMultiplier=1.5`、`propertyPriceMultiplier=0.6`、`totalFunds=10000`、`salary=3000`、`mapId=expanded`、4 玩家启发式大脑。

| 指标 | 第 1 局 | 第 2 局 | 第 3 局 | 第 4 局 | 第 5 局 | 平均 |
|---|---|---|---|---|---|---|
| 结果 | completed | completed | completed | completed | completed | - |
| 回合数 | 156 | 108 | 193 | 166 | 161 | **156.8** |
| 破产数 | 3 | 3 | 3 | 3 | 3 | **3.0/4** |
| 土地购买率 | 79% | 68% | 42% | 74% | 100% | **72.6%** |
| 商店访问率 | 10.1% | 15.9% | 5.5% | 9.4% | 11.3% | **10.4%** |
| 股票总盈亏 | -$3,465 | -$6,309 | -$28,089 | -$15,722 | +$18,806 | **-$6,956** |
| 攻击行为数 | 12 | 12 | 21 | 15 | 10 | **14.0** |

**200 回合观察**：
- **全部 5 局在 200 回合内结束，每局均出现 3/4 玩家破产**，满足“超过 200 回合无人破产视为有问题”的验收标准。
- **买地机制使用充分**：平均 72.6% 空地被购入，但第 3 局仅 42%，说明随机落点与 AI 资金分配会导致个别局土地积累偏慢；即便如此，租金、卡片与攻击行为仍能在 193 回合内结束对局。
- **攻击卡片/道具已被积极使用**：每局 10-21 次攻击，且 100% 指向最富有对手，说明 AI 仇恨目标选择正确。
- **股票盈亏波动极大**：第 3 局合计亏损 $28,089、第 5 局合计盈利 $18,806，5 局平均亏损 $6,956。`stockVolatility=0.6` 放大了股市影响，符合 Playtest 设计目标，但需警惕单局极端亏损导致玩家体验失控。
- **商店访问率整体健康但方差大**：平均 10.4%，但第 3 局仅 5.5%。商店格数量（expanded 地图 6 个）已充足，主要受掷骰随机性影响。可进一步提高 AI 对商店的主动前往意愿。
- **垄断同组数仍为 0**：5 局中没有任何玩家完成整条路段垄断。这说明同组加成机制在 AI 对局中尚未被充分利用，是后续调参重点之一。
...
3. **`planTurn` 缓存导致动作连续失败**：启发式大脑一次性输出整回合计划后，`freePlay.ts` 会缓存该计划并按顺序执行。若前期动作（如买地、商店购买）改变了现金或地块状态，后续计划中的 `upgradeProperty` 可能因资金不足而连续失败 3 次，触发 issue 并强制跳过回合（第 1 局回合 64 出现 2 次）。
   - **修复**：在 `freePlay.ts` 每次动作执行失败后立即清空 `pendingPlans`，让大脑在下一次 `acting` 阶段基于最新状态重新计划，避免用失效计划连续失败。
4. **商店访问率偏低且方差大**：200 回合中商店访问率 5.5%～15.9%，平均 10.4%。虽然整体达标，但个别局低于 8% 的健康下限。
   - **修复方向**：提高启发式大脑对商店格的优先级。当玩家点券不足或卡片/道具较少时，将商店提升到遥控骰子目标的前排；同时在 LLM prompt 中强调“踩商店补充卡片/道具”是中期重要策略。
5. **同组垄断始终未出现**：5 局 200 回合没有玩家完成整条路段垄断，说明当前 AI 换地卡/换房卡使用与关键地块争夺还不够激进。
   - **修复方向**：后续可调高关键地块（差 1 块完成垄断）的购买优先级，并优化换地卡目标选择逻辑，让 AI 更倾向于完成垄断以触发 +20%/+50% 同组加成。
6. **50 回合部分对局无破产**：200 回合验证已确认每局均出现 3/4 破产，无需再提高 `rentMultiplier` 或降低 `salary`；当前 `rentMultiplier=1.5`、`salary=3000` 的组合能在 200 回合内稳定结束对局。
