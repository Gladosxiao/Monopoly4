...
3. **地块形状与色彩体系**：
   ...
   - 功能性地块（起点/命运/机会/商店/税务/医院/监狱/小游戏/公司等）：绘制为**小圆形**（半径约为 tile 短边的 38%），统一浅灰白底（`#f5f7fa`），使用高饱和度类型色加粗描边（3px）；**不绘制标题栏，功能名称直接居中显示在圆内**。
...
4. **标题栏**：property/小矩形功能格顶部约 28% 高度为标题区域；深色底 + 居中白色粗体名称；property 标题栏为分组色加深；标题栏底部带一条区分色细线（property 为白色）；棋子/神明/陷阱图标均使用单格区域定位。
...
9. **面板最小高度**：股市面板最小高度保证可见至少 2 只股票；卡片/道具背包最小高度保证可见至少 1 行道具；折叠后仍保留预览高度，避免高度跳动。
10. **卡片/道具说明**：每个卡片/道具格子右上角显示 `?` 帮助图标，鼠标悬停显示效果说明（优先使用 `title` 属性与自定义 tooltip）。
11. **移动动画**：棋子按 path 逐格移动，每格移动后停顿再继续下一格；停顿阶段应停留在新位置，禁止弹回上一格。
