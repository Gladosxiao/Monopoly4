...
>
> **最近更新**：2026-06-20 — 修复地块 Canvas 渲染：property 使用 pastel 纯色 + 深色顶部标题栏（白色名称），functional 浅灰白底 + 类型色加粗描边；标题栏底部带区分色细线，所有者标识与地产内容排布在标题栏下方。
...
3. **地块色彩体系**：
   - `property` 地块：纯色填充（`GROUP_COLORS`），白/所有者色细描边；顶部标题栏使用 `darkenColor` 对分组色加深。
   - 功能性地块（起点/命运/机会/商店/税务/卡片/点券/新闻/公司/医院/监狱/小游戏）：统一浅灰白底（`#f5f7fa`），使用高饱和度类型色加粗描边（3px）；顶部标题栏使用类型色。
   - 棋子：emoji PNG + 白色描边，不依赖 `player.color` 填充。
   - 建筑/指示物：`BUILDING_COLORS` 暖灰低饱和；神明/陷阱使用独立的绿/红/紫/橙警示色。
4. **标题栏**：每个地块矩形顶部约 28% 高度为标题区域，深色底 + 居中白色粗体名称；property 标题栏为分组色加深，functional 标题栏为类型色；标题栏底部带一条区分色细线（property 为白色，functional 为类型色）；功能性地块左上角附加白色符号图标；棋子/神明/陷阱图标均不进入标题区域。
5. **所有者标识**：标题栏下方显示所有者颜色条 + 名字首字圆标签；地产内容（价格/建筑/等级）自动排布在所有者标识下方。
6. **等级显示**：住宅/连锁店用并列小房子表达等级，中间显示 `Lv.X`；特殊建筑在图标上方显示等级徽章。
